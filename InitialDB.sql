...
CREATE VIEW GETALLPRODUCT_VIEW AS 
SELECT PRODUCT.UNIQUEID PROID, PRODUCT.POSID, PRODUCT.TITLE, PRODUCT.DESCRIPTION PRODESC, PRODUCT.PRICE, PRODUCT.ACTIVE, 
DEPARTMENT.UNIQUEID DEPID, DEPARTMENT.CODE DEPCOD, DEPARTMENT.DESCRIPTION DEPDES, 
STYLE.UNIQUEID STYID, STYLE.CODE STYCOD, STYLE.DESCRIPTION STYDES, 
CATEGORY.UNIQUEID CATID, CATEGORY.CODE CATCOD, CATEGORY.DESCRIPTION CATDES, 
BRAND.UNIQUEID BRAID, BRAND.CODE BRACOD, BRAND.DESCRIPTION BRADES, 
PREMIUMGEAR.UNIQUEID PREID, PREMIUMGEAR.CODE PRECOD, PREMIUMGEAR.DESCRIPTION PREDES, 
CONDITION.UNIQUEID CONID, CONDITION.CODE CONCOD, CONDITION.DESCRIPTION CONDES
FROM 
CONDITION INNER JOIN (
	PREMIUMGEAR INNER JOIN (
		BRAND INNER JOIN (
			CATEGORY INNER JOIN (
				STYLE INNER JOIN (
					DEPARTMENT INNER JOIN PRODUCT 
						ON DEPARTMENT.UNIQUEID = PRODUCT.DEPARTMENT_UID) 
						ON STYLE.UNIQUEID = PRODUCT.STYLE_UID) 
						ON CATEGORY.UNIQUEID = PRODUCT.CATEGORY_UID) 
						ON BRAND.UNIQUEID = PRODUCT.BRAND_UID) 
						ON PREMIUMGEAR.UNIQUEID = PRODUCT.PREMIUMGEAR_UID) 
						ON CONDITION.UNIQUEID = PRODUCT.CONDITION_UID;